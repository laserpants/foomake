...
data Scope = Public | Private | Interface | Default
  deriving (Eq, Show)
...
instance FromJSON IncludeDirectory where
  parseJSON (String s) = pure (IncludeDirectory s Default)
  parseJSON (Object v) =
...
extractDirs Nothing = []
extractDirs (Just group) =
    setScope Public public ++
...
includeDirectories v =
  liftM extractDirs (parseAlias v "includeDirectories" "includeDirs")
